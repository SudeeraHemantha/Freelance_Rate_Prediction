...
def log_prediction_background(payload: Dict[str, Any], predicted_rate: Decimal, ip_address: str):
...
async def predict_rate(
    ...
    try:
        ...
        # 3. Predict total payout
        predicted_payout_val = float(model_pipeline.predict(input_data)[0])
        
        # Guard payouts below zero (bounds constraint)
        if predicted_payout_val < 0.0:
            predicted_payout_val = 50.00

        # 4. Formulate hourly rate metrics
        estimated_hours_float = float(payload.estimated_hours)
        predicted_rate_val = predicted_payout_val / estimated_hours_float

        # Format numeric conversions to Decimal with 2 decimal places precision
        predicted_payout_dec = Decimal(f"{predicted_payout_val:.2f}")
        predicted_rate_dec = Decimal(f"{predicted_rate_val:.2f}")

        # Measure latency
        execution_time_ms = (time.time() - start_time) * 1000

        # 5. Delegate asynchronous database logging to FastAPI background workers
